...
import { Plus, Trash2, Edit2, Package, Search, X, Check, FileQuestion, Clock } from 'lucide-react';
...
const Inventory = () => {
    ...
    const [currentItem, setCurrentItem] = useState({ name: '', quantity: '', category: '' });
    const [searchQuery, setSearchQuery] = useState('');
    const [loading, setLoading] = useState(true);
    const [toast, setToast] = useState({ isVisible: false, message: '', type: 'success', actionText: null, onAction: null });

    const showToast = (message, type = 'success', actionText = null, onAction = null) => {
    ...
    const loadInventory = async () => {
        try {
            ...
            const pendingRequests = requestsData
                .filter(req => req.type === 'INVENTORY_ADD' && (req.status === 'PENDING' || req.status === 'REJECTED'))
                .map(req => {
            ...
            const allItems = [...pendingRequests, ...processedInventory].sort((a, b) =>
        ...
        showToast(
            ...
            async () => {
                try {
                    ...
                    setTimeout(() => showToast(t('updateSuccess') || 'Success', 'success'), 300);
                } catch (error) {
                    console.error('Failed to delete item:', error);
                    setTimeout(() => showToast(t('deleteFailed') || 'Failed to delete', 'error'), 300);
                }
            }
        );
    };
            ...
            if (isEditing) {
                ...
                if (!isAdmin()) {
                    ...
                    setItems([{
                        ...
                        status: 'APPROVED',
                        isRequest: false,
                        requester: newItem.requesterDisplayName || newItem.requesterName || 'Unknown'
                    }, ...items]);
                }
            }
        ...
        <div ref={containerRef} className="max-w-[1500px] mx-auto min-h-screen relative">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    ...
                    <p className="text-text-secondary">Manage your inbound stock and inventory records</p>
                </div>

                <div className="flex gap-4 w-full md:w-auto">
                    <div className="relative group w-full md:w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary group-focus-within:text-accent transition-colors" size={20} />
                        <input
                            type="text"
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-black/5 text-left">
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-left">{t('itemName')}</th>
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-left">{t('category')}</th>
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-left">{t('quantity')}</th>
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-left">{t('status')}</th>
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-left">{t('date')}</th>
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-left">{t('applicant')}</th>
                                <th className="py-4 px-6 text-text-secondary font-medium text-sm text-right">{t('actions')}</th>
                            </tr>
                        </thead>
                    ...
                    <div className="relative bg-bg-surface border border-border rounded-2xl shadow-2xl w-full max-w-md p-6 transform transition-all scale-100">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-text-primary">
                                {isEditing ? (isAdmin() ? t('edit') : t('requestModification')) : t('addItem')}
                            </h2>
                            <button
                                onClick={() => setIsModalOpen(false)}
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1.5">{t('itemName')}</label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1.5">{t('quantity')}</label>
                                <input
                                    type="number"
                                    required
                                    min="1"
                            ...
                            <div className="flex justify-end gap-3 mt-8">
                                ...
                                >
                                    <Check size={18} />
                                    {isEditing ? (isAdmin() ? t('saveChanges') : t('submitRequest') || 'Submit Request') : t('commit')}
                                </button>
                            </div>
                        </form>
                    </div>
