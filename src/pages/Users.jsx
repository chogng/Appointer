...
import { Trash2, Edit2, Search, User, Shield, CheckCircle, XCircle, Plus, Ban, Users as UsersIcon } from 'lucide-react';
...
const Users = () => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            const updatedUser = await apiService.updateUser(editingUser.id, {
                ...
                name: editForm.name,
                expiryDate: editForm.expiryType === 'limited' ? editForm.expiryDate : null,
    ...
    const handleCreate = async () => {
        try {
            if (!createForm.username || !createForm.password || !createForm.name || !createForm.email) {
                showToast(t('fillAllFields') || 'Please fill in all required fields', 'error');
                return;
            }

            const newUser = await apiService.adminCreateUser({
                ...createForm,
                expiryDate: createForm.expiryType === 'limited' ? createForm.expiryDate : null
            });
    ...
    return (
        <div ref={containerRef} className="max-w-[1500px] mx-auto min-h-screen relative">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    ...
                    <p className="text-text-secondary">
                        {t('user_management_subtitle') || 'Manage system users and access controls.'}
                    </p>
                </div>
                ...
                <table className="w-full">
                    <thead className="bg-bg-page/50">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-left text-xs font-semibold text-text-secondary uppercase tracking-wider">Expiry</th>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-bg-surface border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-2">Account Expiry</label>
                                <div className="flex gap-4 mb-3">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={() => setEditForm(prev => ({ ...prev, expiryType: 'permanent' }))}
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={() => setEditForm(prev => ({ ...prev, expiryType: 'limited' }))}
                                            className="text-accent focus:ring-accent"
                                        />
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-2">Account Status</label>
                                <div className="flex gap-4">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={() => setEditForm(prev => ({ ...prev, status: 'ACTIVE' }))}
                                            className="text-accent focus:ring-accent"
                                        />
                                        <span className="text-text-primary flex items-center gap-1"><CheckCircle size={14} className="text-green-500" /> Active</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            type="radio"
                                            ...
                                            onChange={() => setEditForm(prev => ({ ...prev, status: 'DISABLED' }))}
                                            className="text-accent focus:ring-accent"
                                        />
                                        <span className="text-text-primary flex items-center gap-1"><Ban size={14} className="text-red-500" /> Disabled</span>
                                    </label>
                                </div>
                            </div>

                            <div className="pt-4 border-t border-border">
                                <h3 className="text-base font-semibold text-text-primary mb-3">Device Restrictions</h3>
                                {loadingBlocklist ? (
                                    <div className="text-sm text-text-secondary">Loading restrictions...</div>
                                ) : (
                                    <div className="space-y-2 max-h-40 overflow-y-auto">
                                        {devices.map(device => {
                                            const isBlocked = userBlocklist.includes(device.id);
                                            return (
                                                <div key={device.id} className="flex items-center justify-between p-2 bg-bg-page rounded-lg border border-border">
                                                    <span className="text-sm text-text-primary">{device.name}</span>
                                                    <button
                                                        onClick={async () => {
                                                            try {
                                                                if (isBlocked) {
                                                                    await apiService.unblockUserDevice(editingUser.id, device.id);
                                                                    setUserBlocklist(prev => prev.filter(id => id !== device.id));
                                                                } else {
                                                                    await apiService.blockUserDevice(editingUser.id, device.id);
                                                                    setUserBlocklist(prev => [...prev, device.id]);
                                                                }
                                                            } catch (error) {
                                        ...
                                        {devices.length === 0 && <div className="text-sm text-text-secondary">No devices found.</div>}
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {isCreateModalOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-bg-surface border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
                        <h2 className="text-xl font-bold text-text-primary mb-4">{t('addUser') || 'Add User'}</h2>
                        ...
                        <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1">Username <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1">Display Name <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1">Email <span className="text-red-500">*</span></label>
                                <input
                                    type="email"
                                ...
                                <div className="flex gap-4 mb-3">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={() => setCreateForm(prev => ({ ...prev, expiryType: 'permanent' }))}
                                            className="text-accent focus:ring-accent"
                                        />
                                        <span className="text-text-primary">Permanent</span>
                                    </label>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={() => setCreateForm(prev => ({ ...prev, expiryType: 'limited' }))}
                                            className="text-accent focus:ring-accent"
                                        />
                                        <span className="text-text-primary">Limited Time</span>
                                    </label>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-bg-surface border border-border rounded-xl p-6 w-full max-w-sm shadow-2xl">
                        <h2 className="text-lg font-bold text-text-primary mb-2">Confirm Deletion</h2>
