...
import { Calendar, Clock, CheckCircle, Activity, Search, Trash2, MessageSquare, Check, X, User, Package } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const stats = [
        ...
        { label: t('reservedHours'), value: `14 ${t('hours')}`, icon: Clock, color: '#34C759' },
    ...
    const fetchPendingUsers = useCallback(async () => {
        try {
            ...
            setPendingUsers(users.filter(u => u.status === 'PENDING').map(u => ({ ...u, msgType: 'USER_REGISTRATION' })));
        } catch (error) {
    ...
    const fetchRequests = useCallback(async () => {
        try {
            const reqs = (await apiService.getRequests()).filter(r => r.status === 'PENDING');
            let visibleRequests = reqs;
            if (user?.role !== 'ADMIN' && user?.role !== 'SUPER_ADMIN') {
                visibleRequests = reqs.filter(r => r.requesterId === user?.id);
            }
            setRequests(visibleRequests.map(r => ({ ...r, msgType: 'INVENTORY_REQUEST' })));
        } catch (error) {
    ...
    const handleReject = async (msg) => {
        const confirmMsg = msg.msgType === 'USER_REGISTRATION'
        ...
        <div ref={containerRef} className="max-w-[1500px] mx-auto relative min-h-screen">
            ...
            <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <Card variant="glass" className="p-0 overflow-hidden h-[500px] overflow-y-auto">
                        {logs.length > 0 ? (
                ...
                <div>
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h2 className="text-xl font-serif font-medium text-text-primary">{t('recentMessages') || 'Recent Messages'}</h2>
                    </div>
                    <Card variant="glass" className="p-0 overflow-hidden h-[500px] overflow-y-auto">
                        {messages.length > 0 ? (
                            <div className="divide-y divide-border-subtle">
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-[200px]">
                                            <div className="w-10 h-10 rounded-full bg-bg-200 flex items-center justify-center text-text-primary font-medium shrink-0 group-hover:bg-accent/10 group-hover:text-accent transition-colors">
                                                {msg.msgType === 'USER_REGISTRATION' ? (msg.name?.[0] || '?') : <Package size={18} />}
                                            </div>
                                            <div>
                                                <div className="text-sm font-medium text-text-primary group-hover:text-accent transition-colors">
                                                    {msg.msgType === 'USER_REGISTRATION' ? msg.name : msg.requesterName}
                                                    {msg.msgType === 'USER_REGISTRATION' && <span className="text-text-tertiary text-xs"> ({msg.username})</span>}
                                                </div>
                                                <div className="text-xs text-text-secondary">
                                                    {msg.msgType === 'USER_REGISTRATION' ? 'Applied for account' : 'Inventory Request'}
                                                </div>
                                                {msg.email && (
                                                    <div className="text-xs text-text-tertiary">
                                                        {msg.email}
                                                ...
                                                try {
                                                    ...
                                                    return (
                                                        <span className="flex items-center gap-2 bg-bg-200/50 px-3 py-1.5 rounded-lg border border-border/50">
                                                            ...
                                                            <span className="text-xs px-1.5 py-0.5 rounded bg-accent/10 text-accent font-medium">x{quantity}</span>
                                                        </span>
                                                    );
                                                } catch (e) {
                                                    return null;
                                        ...
                                        <div className="flex items-center gap-2">
                                            {(user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') ? (
                                                <>
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleApprove(msg); }}
                                                    ...
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleReject(msg); }}
                                                ...
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleRevoke(msg); }}
            ...
            <Modal
                ...
                title={selectedMessage?.msgType === 'USER_REGISTRATION' ? 'Application Details' : 'Request Details'}
                footer={
                    <>
                        {(user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN') ? (
                            <>
            ...
            >
                {selectedMessage && selectedMessage.msgType === 'USER_REGISTRATION' && (
                    <div className="space-y-4">
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h3 className="text-lg font-medium text-text-primary">{selectedMessage.name}</h3>
                                <p className="text-text-secondary">{selectedMessage.username}</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-bg-200/50 rounded-xl">
                                <div className="text-xs text-text-tertiary mb-1">Student ID</div>
                ...
                {selectedMessage && (selectedMessage.msgType === 'INVENTORY_REQUEST' || selectedMessage.type === 'INVENTORY_ADD') && (
                    <div className="space-y-4">
                        <div className="flex items-center gap-4 mb-6">
                            <div className="w-16 h-16 rounded-full bg-bg-200 flex items-center justify-center text-text-primary text-2xl font-serif">
                                <Package size={32} />
                            ...
                            <div>
                                ...
                                <p className="text-text-secondary">Requested by {selectedMessage.requesterName}</p>
                            </div>
                        </div>

                        <div className="bg-bg-200/50 rounded-xl p-4">
                            <h4 className="text-sm font-medium text-text-primary mb-3">
                                {selectedMessage.type === 'INVENTORY_ADD' ? 'Item Details' : 'Changes Breakdown'}
                            </h4>
