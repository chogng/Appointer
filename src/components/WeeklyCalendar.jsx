...
const WeeklyCalendar = ({
  ...
  const isTimeOpen = (time) => {
    ...
    const [startH, startM] = (device.openTime.start || "00:00")
      .split(":")
      .map(Number);
    const [endH, endM] = (device.openTime.end || "24:00")
      .split(":")
      .map(Number);
  ...
  const getOpenTimeRange = useCallback(() => {
    const [startH, startM] = (device?.openTime?.start || "00:00")
      .split(":")
      .map(Number);
    const [endH, endM] = (device?.openTime?.end || "24:00")
  ...
  }, [device?.openTime?.end, device?.openTime?.start]);

  const openPopoverForDraft = useCallback(
    (targetEvent = draftEvent) => {
      ...
      const dayIndex = weekDays.findIndex(
        (day) => format(day, "yyyy-MM-dd") === targetEvent.date,
      );
      ...
      top = Math.max(
        10,
        Math.min(top, window.innerHeight - popoverHeight - 10),
      );
      left = Math.max(
        10,
        Math.min(left, window.innerWidth - popoverWidth - 10),
      );
  ...
  useEffect(() => {
    const handleGlobalMouseUp = () => {
      ...
      if (isDragging && selectionDraft) {
        ...
        const h1 = parseInt(
          selectionDraft.startSlot.split("-")[0].split(":")[0],
        );
        ...
        const newDraft = {
          date: selectionDraft.date,
          timeSlot: combinedSlot,
      ...
      if (draftInteraction && gridRef.current) {
        ...
        if (draftInteraction.type === "move") {
          ...
          const [startStr, endStr] =
            draftInteraction.startData.timeSlot.split("-");
          ...
          newStartH = Math.max(
            openStart,
            Math.min(openEnd - duration, newStartH),
          );
          ...
          const dayOpen =
            !device?.openDays ||
            device.openDays.includes(isoDay) ||
            device.openDays.includes(jsDay);
          ...
          <div className="grid grid-cols-[60px_8px_1fr] border-border min-h-[20px] bg-bg-surface">
            ...
            <div className="grid grid-cols-7 divide-x divide-border relative border-b border-border">
              ...
              {weekDays.map((day, i) => (
                <div
                  key={i}
                  className="relative h-full first:border-l first:border-border"
                >
          ...
          <div className="grid grid-cols-[60px_8px_1fr] min-h-full">
            ...
            <div className="bg-bg-surface select-none" aria-hidden="true">
              {hours.map((hour) => (
                <div
                  key={hour}
                  className="h-[50px] border-b border-border"
                ></div>
              ))}
            </div>
            ...
            <div
              ref={gridRef}
              className="grid grid-cols-7 divide-x divide-border relative border-b border-border"
            >
              ...
              {weekDays.map((day, dayIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {selectionDraft &&
                      selectionDraft.date === dateStr &&
                      (() => {
                        const h1 = parseInt(
                          selectionDraft.startSlot.split("-")[0].split(":")[0],
                        );
                        const h2 = parseInt(
                          selectionDraft.endSlot.split("-")[0].split(":")[0],
                        );
                        const minH = Math.min(h1, h2);
                        const maxH = Math.max(h1, h2);
                        const top = minH * 50;
                        ...
                        const colorStyles =
                          EVENT_COLORS[selectionDraft.color] ||
                    ...
                    {draftInteraction?.type === "move" &&
                      draftInteraction.startData.date === dateStr &&
                      (() => {
                        const [startStr, endStr] =
                          draftInteraction.startData.timeSlot.split("-");
                        const startH = parseInt(startStr.split(":")[0]);
                        ...
                        const colorStyles =
                          EVENT_COLORS[draftInteraction.startData.color] ||
                          EVENT_COLORS.default;
                        return (
                          <div
                    ...
                    {draftEvent &&
                      draftEvent.date === dateStr &&
                      (() => {
                        const [startStr, endStr] =
                          draftEvent.timeSlot.split("-");
                        const startH = parseInt(startStr.split(":")[0]);
                        const endH = parseInt(endStr.split(":")[0]);
                        const top = startH * 50;
                        const height = (endH - startH) * 50;
